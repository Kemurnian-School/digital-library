...
<main class="p-4 max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-black">Edit Student</h1>
    </div>
    ...
    <form action="{{ route('admin.students.update', $student) }}" method="POST" class="bg-white border border-gray-300 rounded p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select name="level"
                    ...
                    <option value="sd" {{ old('level', $student->level) == 'sd' ? 'selected' : '' }}>SD</option>
                    <option value="smp" {{ old('level', $student->level) == 'smp' ? 'selected' : '' }}>SMP</option>
                    <option value="sma" {{ old('level', $student->level) == 'sma' ? 'selected' : '' }}>SMA</option>
                </select>
                @error('level')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
